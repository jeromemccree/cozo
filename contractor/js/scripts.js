...
function validateAuth() {
    ...
    if (email.value.trim() == "" || !email.value.match(emailformat) || email.value.trim().length > 35) {
        email.style.border = "solid 1px red";
        document.getElementById("AinvalidEmail").style.visibility = "visible";
        return false;
    }

    if (password.value.trim() == "" || password.value.trim().length < 5 || password.value.trim().length > 12) {
...
function validateRegister() {
    ...
    axios({
        ...
        if (response.data == "Sorry please enter info") {
            ...
            document.cookie = "ContractorJWT=" + response.data;
            return false;
