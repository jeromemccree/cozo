...
// recBuf is exactly equivalent to rec, except it uses a buffer supplied by
// the caller to avoid an allocation.
func (cn *conn) recBuf(r *readBuf) byte {
	for {
		t, err := cn.recvMessage(r)
		if err != nil {
			panic(err)
...
// rec receives a message from the backend, panicking if an error occurs
// while attempting to read it.  All asynchronous messages are ignored, with
// the exception of ErrorResponse.
func (cn *conn) rec() (t byte, r *readBuf) {
	r = &readBuf{}
	t = cn.recBuf(r)
	return t, r
}
...
func (cn *conn) postExecuteWorkaround() {
	...
	// CommandComplete), so we push the message into the conn struct; rec
